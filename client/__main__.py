...
class Client:
    ...
    def recv_card_info(self):
        ...
        self.user.score = self.users_score[self.client_player]
        
        self.cards_num = self.recv_data()
        
        self.played_cards = self.recv_data()
        self.user.played_card = self.played_cards[self.client_player]
    ...
    def run(self):
        ...
        while True:
            self.recv_card_info()
            # UI
            last_player = utils.last_played(self.played_cards, self.now_player)
            ...
            main_interface(self.users_name, self.client_player, self.users_score, self.cards_num, self.played_cards,
                           self.user, self.now_score, self.now_player, self.head_master, last_player,
                           self.his_now_score, self.his_last_player, self.is_start)
            # 记录历史信息
            ...
            if self.is_player and self.client_player == self.now_player:
                self.now_score += playing(self.user, last_player, self.client_player, self.played_cards, self.client)    
                self.send_card_info()
